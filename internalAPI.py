...
class internalAPI(object):
    ...
    def processEvent(self, destinations, event_name, start_time, end_time, event_description='', event_location='', event_picture=''):
        ...
        if cherrypy.session['userdata'].status:
            ...
            for destination in destinations:
                data = {'sender': cherrypy.session['userdata'].username, 'destination': destination, 'event_name': event_name, 'event_description': event_description,
    ...
    def sendMessage(self, message, attachments):
        if 'userdata' not in cherrypy.session:
            raise cherrypy.HTTPRedirect("login")
        if cherrypy.session['userdata'].status:
            data = {'sender': unicode(cherrypy.session['userdata'].username), 'destination': unicode(cherrypy.session['userdata'].currentChat), 'message': unicode(
            ...
            try:
                ...
                stuff = {'sender': cherrypy.session['userdata'].username, 'destination': cherrypy.session['userdata'].currentChat, 'file': attachments,
